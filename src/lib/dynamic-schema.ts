import type { DynamicTableName, JsonValue } from '../types';
...
function normalizeFieldName(fieldName: string): string {
  ...
  const prefixed = /^[0-9]/.test(safe) ? `f_${safe}` : safe;

  if (RESERVED_COLUMNS.has(prefixed)) {
    return `field_${prefixed}`;
  }

  return prefixed.slice(0, 28);
}

export function dynamicColumnName(fieldName: string): string {
...
export async function ensureDynamicColumns(
  ...
  for (const fieldName of uniqueFieldNames) {
    const columnName = dynamicColumnName(fieldName);
...
export function serializeDynamicColumnValue(value: JsonValue): string | null {
  if (value === null) {
    return null;
  }
...
export function extractDynamicColumns(
  ...
  Array.from(new Set(Array.from(fieldNames).filter(Boolean)))
    .sort((left, right) => left.localeCompare(right))
    .forEach((fieldName) => {
      const columnName = dynamicColumnName(fieldName);
      ...
      values[fieldName] = value === null ? null : String(value);
    });
